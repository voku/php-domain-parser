...
final class ParserTest extends TestCase
{
    ...
    {
        static::assertFalse($this->parser->isSuffixValid('www.example.faketld'));
        static::assertFalse($this->parser->isSuffixValid('example.example'));
    }
    ...
    {
        static::assertTrue($this->parser->isSuffixValid('www.example.com'));
        static::assertTrue($this->parser->isSuffixValid('www.example.co.uk'));
        static::assertTrue($this->parser->isSuffixValid('www.example.рф'));
        static::assertTrue($this->parser->isSuffixValid('example.com.au'));
    }
    ...
    public function testParseUrl($url, $publicSuffix, $registrableDomain, $subdomain, $hostPart)
    {
        ...
        static::assertInstanceOf('\Pdp\Uri\Url', $pdpUrl);
    }
    ...
    {
        $pdpUrl = $this->parser->parseUrl($url);
        static::assertSame($hostPart, (string) $pdpUrl->getHost());

        $pdpHost = $this->parser->parseHost($hostPart);
        static::assertInstanceOf('\Pdp\Uri\Url\Host', $pdpHost);
        static::assertSame($hostPart, $pdpHost->__toString());
    }
    ...
    {
        $pdpUrl = $this->parser->parseUrl($url);
        static::assertSame($publicSuffix, $pdpUrl->getHost()->getPublicSuffix());
        static::assertSame($publicSuffix, $this->parser->getPublicSuffix($hostPart));
    }
    ...
    {
        $pdpUrl = $this->parser->parseUrl($url);
        static::assertSame($registrableDomain, $pdpUrl->getHost()->getRegistrableDomain());
        static::assertSame($registrableDomain, $this->parser->getRegistrableDomain($hostPart));
    }
    ...
    {
        $pdpUrl = $this->parser->parseUrl($url);
        static::assertSame($subdomain, $pdpUrl->getHost()->getSubdomain());
        static::assertSame($subdomain, $this->parser->getSubdomain($hostPart));
    }
     ...
     * @param $hostPart
     */
    public function testTestpdpParseUrlCanReturnCorrectHost($url, $publicSuffix, $registrableDomain, $subdomain, $hostPart)
    {
        static::assertSame(
            $hostPart,
